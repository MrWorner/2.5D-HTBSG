...
{
public class MG_MouseS_SettlementPlacer : MG_MouseStrategy
{
    #region Поля
    ...
    [PropertyOrder(-1), BoxGroup("ТРЕБОВАНИЯ"), Required(InfoMessageType.Error), SerializeField] private MG_SettlementManager _settlementManager;//[R] фабрика поселений
    [PropertyOrder(-1), BoxGroup("ТРЕБОВАНИЯ"), Required(InfoMessageType.Error), SerializeField] private MG_Player _side;//[R] сторона
    [PropertyOrder(-1), BoxGroup("ТРЕБОВАНИЯ"), Required(InfoMessageType.Error), SerializeField] private MG_SettlementType _type;//[R] тип поселения
    ...
    private void Awake()
    {
        ...
        if (_cameraObject == null) Debug.Log("<color=red>MG_MouseS_SettlementPlacer Awake(): объект '_mainCameraGameObject' не прикреплен!</color>");
        if (_map == null) Debug.Log("<color=red>MG_MouseS_SettlementPlacer Awake(): объект '_map' не прикреплен!</color>");
        if (_settlementManager == null) Debug.Log("<color=red>MG_MouseS_SettlementPlacer Awake(): объект '_settlementManager' не прикреплен!</color>");
        if (_side == null) Debug.Log("<color=red>MG_MouseS_SettlementPlacer Awake(): объект '_side' не прикреплен!</color>");
        if (_type == null) Debug.Log("<color=red>MG_MouseS_SettlementPlacer Awake(): объект '_type' не прикреплен!</color>");
    }
    ...
    private void OnClickCell(MG_HexCell cell, bool isLeftClick)
    {
        ...
        if (isLeftClick)
        {
            ...
            _settlementManager.Create(cell, _side, _type);
