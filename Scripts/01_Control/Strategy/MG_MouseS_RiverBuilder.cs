...
{
public class MG_MouseS_RiverBuilder : MG_MouseStrategy
{
    #region Поля
    ...
    [BoxGroup("Типы клеток, которые не могут содержать реки"), SerializeField] private List<GroundType> _notAllowedCellTypes = new List<GroundType>() { GroundType.Water, GroundType.Mountain }; //Типы клеток, которые не могут содержать реки
    #endregion Поля

    #region Поля: Необходимые модули и объекты для заполнения
    ...
    [PropertyOrder(-1), BoxGroup("ТРЕБОВАНИЯ"), Required(InfoMessageType.Error), SerializeField] private MG_HexRiverManager _riverBuilder;//менеджер рек
    #endregion Поля: Необходимые модули и объекты для заполнения
    ...
    private void Awake()
    {
        ...
        if (_cameraObject == null)
            Debug.Log("<color=red>MG_MouseS_RiverBuilder Awake(): объект '_mainCameraGameObject' не прикреплен!</color>");
        if (_map == null)
            Debug.Log("<color=red>MG_MouseS_RiverBuilder Awake(): объект '_map' не прикреплен!</color>");

        if (_riverBuilder == null)
            Debug.Log("<color=red>MG_MouseS_RiverBuilder Awake(): объект '_settlementManager' не прикреплен!</color>");
    }
