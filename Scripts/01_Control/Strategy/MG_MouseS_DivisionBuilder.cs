...
{
public class MG_MouseS_DivisionBuilder : MG_MouseStrategy
{
    #region Поля
    ...
    [BoxGroup("Настройки"), SerializeField] private List<GroundType> _notAllowedCellTypes = new List<GroundType>() { GroundType.Water, GroundType.Mountain };//на каких типах поверхностей нельзя создать дивизию
    #endregion Поля
    ...
    [PropertyOrder(-1), BoxGroup("ТРЕБОВАНИЯ"), Required(InfoMessageType.Error), SerializeField] private MG_UnitObjectFactory _unitFactory;//[R] фабрика дивизий
    ...
    private void Awake()
    {
        ...
        if (_cameraObject == null) Debug.Log("<color=red>MG_MouseS_DivisionBuilder Awake(): объект '_mainCameraGameObject' не прикреплен!</color>");
        if (_map == null) Debug.Log("<color=red>MG_MouseS_DivisionBuilder Awake(): объект '_map' не прикреплен!</color>");
        if (_unitFactory == null) Debug.Log("<color=red>MG_MouseS_DivisionBuilder Awake(): объект '_settlementManager' не прикреплен!</color>");
        if (_side == null) Debug.Log("<color=red>MG_MouseS_DivisionBuilder Awake(): объект '_side' не прикреплен!</color>");
    }
    ...
    private void OnClickCell(MG_HexCell cell, bool isLeftClick)
    {
        ...
        if (isLeftClick)
        {
            ...
            {
                _unitFactory.Create(cell, _side);
        ...
        else
        {
            if (cell.IsTaken)
            {
                var division = cell.Division;
                division.RemoveFromGame();
                MG_PathAreaFixer.FixAfterCellUpdate(cell);
            }
        }
